...
To account for this and obtain a summary of length $\wordbudget$, 
...
%?We outline the dependencies of $\xHid_i$ in each of the
%?models as,
%?\begin{itemize}
%?    %\item (\clext) --- $\sentEmb_{i-1}, $
%?    \item left and right partial contextual sentence embeddings (? and ?)
%?    \item a mixture of all encoder left and right partial contextual embeddings, and the $i^\textrm{th}$ decoder left and right partial contextual embeddings
%?    \item the mean left and right partial contextual embeddings 
%?\end{itemize}
